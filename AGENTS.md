...
| 3 | **Image Hygiene** | 每次模型调用前滚动剪除历史截图：只有最新 `PHONE_AGENT_IMAGE_KEEP`（默认 2）条含图消息保留图片块；工具成功总是回传新图。若必须剪除的图片/OBS 块带原生签名且无合法投影，micro 在修改前全量预检并以 `native_context_pruning_conflict` 明确失败，不丢/搬签名、不多保留旧图；同消息其他块的签名不阻止无签名旧图清理。实现：`middleware/images.py`。 |
...
| 6 | **Trace Redaction** | 每个 model/tool 事件写 `<trace_dir>/<run_id>.jsonl`：文本 >64 字截断、敏感子串经 `config/redact.py` 脱敏、截图 base64 永不落盘（只记 `screen_seq` + 字节数）。 |
| 7 | **Device via DeviceFactory** | 所有设备操作经 `DeviceFactory` → `phone_agent/adb/`；无直接 ADB 调用。 |
| 8 | **Config via V2Config** | CLI 覆盖 > shell env > `.env`（`PHONE_AGENT_` 前缀）> 默认；无硬编码端点/密钥/参数。模型层在 env 之下加一层单层 `models.json`（`PHONE_AGENT_MODELS_FILE` > 项目根 `.taskwizard.models.json`）+ 可选 `roles` 段；precedence 与字段见 `pages/configuration.md`。有效 actor `contextWindow` 驱动 compact，声明的 `maxTokens` 进入 transport。显式未知 provider 与无法构建的显式引用仍可见失败，绝不静默换网关（唯显式配置 `PHONE_AGENT_FALLBACK_MODEL` 时才按该备用降级一次）；运行时装配对缺失/损坏/部分坏声明逐项跳过并写脱敏 `declaration_warnings`（env gateway 保留），严格解析函数保留供显式校验。Anthropic 尾部 `SystemMessage` 提升是唯一已测行为；无证据不声称 Google 等价。 |
...
| 13 | **Token Budget** | 成本以 token 计（`middleware/budget.py`）：累计 `usage_metadata` input+output（无 usage 回退 `(input+output)//4`、CJK 感知、+1500/图），跨压缩仍有效。`PHONE_AGENT_TOKEN_BUDGET`（默认 1M）耗尽硬停（`token_budget_exhausted`）；`PHONE_AGENT_TOKEN_WARN_REMAINING`（默认 100k）注入一次性余量提醒；`PHONE_AGENT_MAX_STEPS`（默认 100）只是 `loop_fuse` 保险丝。 |
| 14 | **Context Work Target + Auto-Compact** | `middleware/compact.py` 作为 `model/pre_request` 监听器最先运行，先完成最新 K 图/marks 的确定性 micro。完整输入软目标 `PHONE_AGENT_CONTEXT_WORK_TARGET` 默认 32k（0 关闭软目标），压缩后目标比例默认 0.7；物理窗口仍保留 T1（0.75）提醒与 T2（0.92）触发。原始任务、当前 TaskDoc、最新完整 AI/所有 sibling 工具组、活图/marks 与 opaque 依赖不为软目标截断；不改写 HTML 参数。摘要前检查摘要模型容量，必要时按完整组分段+合并；每次逻辑摘要调用前检查 run token 预算，单次压缩至多 8 次（不声称限制 SDK HTTP 重试）。候选重新计数并检查净缩减，失败保留已 micro 的基线，绝不提交半摘要。软目标不可达不能阻止合法物理容量修复：保留保护组并报告 soft_target_unattainable；已超容量时，恢复准入的正净缩减优先于普通软收益门槛。标准块 extras 内的签名/加密元数据递归识别与估计；SDK function-call id 对照表不永久 pin 历史；除此及已有结构化调用重复字段外，未知非空 additional_kwargs 默认保留并非零估计。Provider 已估算工具定义时不重复加 schema reserve；已知实际模型容量不能被较大 actor override 放宽，未知估算与已知超限分开。总开关 `PHONE_AGENT_COMPACT`。**消息协议契约**：`model/pre_request` 监听器是**纯全列表变换**——绝不发 `RemoveMessage`、绝不返回 `jump_to`；列表变化时由桥接中间件统一铸出唯一合法 `[RemoveMessage(REMOVE_ALL), *result]`。 |
