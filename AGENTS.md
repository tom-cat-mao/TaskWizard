...
| 18 | **Capability Mount + Plugins** | 十二个能力经五接缝挂载（middleware / tool / prompt / hook / CLI，另有 `register_service`）；插件是外部代码，`plugin add` 即执行授权。 | [#capability-mount](pages/plugins.md#capability-mount)、[#plugin-authorization](pages/plugins.md#plugin-authorization) |
