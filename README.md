...
- OpenAI 兼容网关可用 `compat.requestApi=auto|chat|responses` 明确协议（默认保留 SDK auto）；支持的端点可显式启用 `cachePolicy=stable-prefix`。模型 context 支持可由插件提供，默认不猜网关缓存能力，详见[配置参考](pages/configuration.md)与[插件开发](pages/plugins.md)。
