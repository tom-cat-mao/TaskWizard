...
- **手册会攒下历史叙述**：`pages/*.md` 里的"以前 / 不再 / 改名为 / 现已 / 旧版 / 新版 / 原本 / 曾经"把
  页面钉在某个改动日期上；读者从没见过旧状态，比较句对他零信息，却会随每次改动留下。落地时实扫：3 页
  共 6 处未处理命中（`architecture.md` 2、`configuration.md` 3、`memory.md` 1）。它们未必都该删——"耗尽后
  不再付摘要调用"是规则——问题在于**读到时已无从判断是规则还是历史**。
...
- **bash 围栏块是"复制即可执行"的承诺，却没有进程解析过它**：占位符与残缺引号在 review 里读不出来，
  ...
  `<serial>` / `<run_dir>`（分别落地于 2026-09-12 与 2026-09-13 的改动），其中一块还留着 ANSI 残留 `[m`。
...
- **收益**：四类腐化从"靠自觉"变成每个 PR 的机器事实，且不需要新增 workflow、job 或依赖；`tests/docs`
  从 40 项涨到 149 项。落地当次即抓到真实问题：4 个不可执行的 bash 块（占位符 + ANSI 残留）被改成可
  粘贴命令，3 个页面 6 处变迁词被逐个甄别（5 处确认为行为规则、加行级豁免，1 处改写为现状陈述），另有
  1 处改写与 4 处豁免把同类句子一次收口；笔记门禁对既有 8 篇笔记零告警（格式已一致，门禁是防回流），
