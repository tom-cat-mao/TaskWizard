...
本页只列**用户相关**的能力状态与限制；入库可查的证据是 merge 记录与代码本身，批次级的授权、评审与验收原始
记录是维护者本机的私有文档、不入库（内部口径见 `docs/future-roadmap.md` 的「证据入口」节）。
...
| 实机诊断 | Case + 正式 runner 子进程；真实事件派生状态（请求停止 ≠ 已结束）；证据流 + 截图落盘 + 离线逐步回放 HTML（三层裁定：harness 终局 / Case 验收 / 诊断推断） |
...
| prefix-cache 优化（任务板版本化 + 图片批量折叠） | 明确延期不做 |
