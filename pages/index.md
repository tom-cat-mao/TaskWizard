...
| App 名解析 | 四层解析：归一化 → 多路候选（精确/词汇/拼音/嵌入）→ 先验排序 → 证据分型三态；弱证据只给候选不执行（`learned` 别名带成功计数、命中类型不是 clarify 类时例外，可单独自动解析） |
...
- [记忆](memory.md)：App-KB、episode 经验档案与语义回想
- [自进化](evolution.md)：蒸馏分级、受控回注与离线 replay 评估
